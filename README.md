...
That draws `/users/sign_in`, `/users/sign_up`, and the complete password-reset
flow from `recording_studio_user/auth/*` — viewport-centered Flatpack forms, no
Card. The first screen is email only (**Continue with email**), then social
providers. Password or OTP is the second screen, driven by `primary_login_type`
(default `:email`). The confirmations mapping gives Devise's resend screen the
same chrome when `:confirmable` is enabled. When
`recording_studio_site_settings` is installed, the site mark appears above
...
RecordingStudioUser.configure do |config|
  ...
  # config.auth_logo = :square # or :wide
end
```
...
`auth_logo` nominates the Site Settings mark on those unsigned screens:

- `:square` (default) — app icon as a Flatpack Avatar
- `:wide` — wide mark as a proportioned `<img>`. Missing wide file omits the mark.
...
The dummy mounts Users auth with `recording_studio_user_auth_for :users`. Login, sign up, password reset, and resend confirmation are gem screens — the gem auth layout centers the viewport (`min-h-dvh`) and the shared `_shell` sets the `max-w-sm` column with centered titles, no Card, no Remember me, no seed Badge. First screens are email only with **Continue with email**, then `continue_with_providers`. Password is the second screen when `primary_login_type` is `:email` (the default), with a **Forgot your password?** link into the reset flow. Login uses `login_title` (default **Welcome back**). Sign-up confirmation appears only when `require_password_confirmation` is true. Dummy development credentials keep Google live. Deep-link OTP lives on `/users/sign_in/otp` and `/users/sign_up/otp`. Signed-in pages use core `recording_studio/default_layout` via `RecordingStudio::UsesDefaultLayout` (PageNav back/close). Auth pages use the gem layout `recording_studio_user/auth` with `html data-theme="rounded"`; the dummy's padded `layouts/application` main is no longer in that stack. Dummy does not copy or override core's default layout. Core puts `data-theme="rounded"` on `body`; Flatpack named-theme tokens resolve on `html` / `:root`, so dummy's `recording_studio/_default_layout_head` sets `document.documentElement.dataset.theme` to `rounded` so primary buttons inherit charcoal, not `:root` blue. Dummy pins Flatpack `v0.1.186` so the TnC continue-notice modal can render.

Dummy also installs released `recording_studio_terms_and_conditions` `v0.6.2` as an end-to-end signup demo. That pin lives in `test/dummy/Gemfile` only. The Users gemspec and root Gemfile stay free of TnC. The path gem satisfies TnC’s `recording_studio_user >= 0.12.1` requirement. Dummy seeds one live Terms record under **My workspace**. Users then detects the installed gem and the live record, so create-password shows the continue-notice without a host `extra_fields` override. Dummy overrides TnC’s Agree template to drop the “Terms updated” alert and tighten the checkbox gap.
