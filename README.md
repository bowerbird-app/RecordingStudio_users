...
Show puts **Edit** in the PageTitle actions slot (not in the card) and one Flatpack elevated Card: Avatar left of unlabeled name, email, and time zone. Empty photos use Avatar's person icon, not initials. An inner Grid `cols: 2` stacks that row at phone width (`grid-cols-1` until `md`). An outer Grid `cols: 2` keeps the card to one cell on a wide screen. There is no show subtitle and no city field. Edit renders `render_parent_attachment(..., shape: :circle, size: :xl)` for Avatar plus the icon-only camera, then stacked fields, in a Flatpack Grid `cols: 2` so the form sits in one cell on large screens. First name, Last name, and Time zone stay full-width rows — not side by side. Update profile and Cancel are two separate Flatpack buttons sitting next to each other, not a ButtonGroup. Edit keeps the plain subtitle: "Change your name, time zone, or photo."
