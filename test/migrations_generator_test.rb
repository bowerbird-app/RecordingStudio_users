...
class MigrationsGeneratorTest < Minitest::Test
  ...
  def test_template_creates_identities_table
    template = File.read(
      File.expand_path(
        "../lib/generators/recording_studio_user/migrations/templates/" \
        "create_recording_studio_user_identities.rb.tt",
        __dir__
      )
    )
    ...
    refute_includes template, "recordable"
  ...
  def test_generator_skips_when_the_host_already_has_the_migration
    ...
    Dir.mktmpdir do |dir|
      ...
      File.write(
        File.join(dir, "db/migrate/20260829000000_create_recording_studio_user_identities.rb"),
        "# existing\n"
      )
