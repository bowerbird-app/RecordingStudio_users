...
class RenameVerificationTest < Minitest::Test
  ...
  def test_memberships_controller_exists
    path = File.join(@root, "app", "controllers", @gem_name, "memberships_controller.rb")
    assert File.exist?(path),
           "Memberships controller should exist at #{path}"
  end

  def test_memberships_controller_has_correct_module
    path = File.join(@root, "app", "controllers", @gem_name, "memberships_controller.rb")
    content = File.read(path)
    assert_match(/^module #{@pascal_name}$/, content,
                 "Memberships controller should be in module #{@pascal_name}")
  end
