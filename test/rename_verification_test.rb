...
class RenameVerificationTest < Minitest::Test
  ...
  def test_onboarding_controller_exists
    path = File.join(@root, "app", "controllers", @gem_name, "onboarding_controller.rb")
    assert File.exist?(path),
           "Onboarding controller should exist at #{path}"
  end

  def test_onboarding_controller_has_correct_module
    path = File.join(@root, "app", "controllers", @gem_name, "onboarding_controller.rb")
    content = File.read(path)
    assert_match(/^module #{@pascal_name}$/, content,
                 "Onboarding controller should be in module #{@pascal_name}")
  end
