...
class EngineTest < Minitest::Test
  def setup
    @original_configuration = RecordingStudioUser.instance_variable_get(:@configuration)
    RecordingStudioUser.instance_variable_set(:@configuration, RecordingStudioUser::Configuration.new)
  end

  def teardown
    RecordingStudioUser.configuration.hooks.clear!
    RecordingStudioUser.instance_variable_set(:@configuration, @original_configuration)
  end
  ...
  def test_before_and_after_initialize_initializers_run_hooks
    ...
    RecordingStudioUser.configuration.hooks.before_initialize { |_engine| before_called = true }
    RecordingStudioUser.configuration.hooks.after_initialize { |_engine| after_called = true }

    find_initializer("recording_studio_user.before_initialize").block.call(Object.new)
    find_initializer("recording_studio_user.after_initialize").block.call(Object.new)
  ...
  def test_load_config_merges_config_sources_and_runs_on_configuration_hook
    hook_called = false
    hook_payload = nil
    RecordingStudioUser.configuration.hooks.on_configuration do |cfg|
      hook_called = true
      hook_payload = cfg
    end

    xcfg = Struct.new(:recording_studio_user).new({ enable_feature_x: true })
    ...
    find_initializer("recording_studio_user.load_config").block.call(app)

    assert hook_called
    assert_equal RecordingStudioUser.configuration, hook_payload
    assert_equal "from_yaml", RecordingStudioUser.configuration.api_key
    assert_equal 12, RecordingStudioUser.configuration.timeout
    assert_equal true, RecordingStudioUser.configuration.enable_feature_x
  end
  ...
  def test_load_config_handles_errors_and_each_pair_fallback
    ...
    xcfg = Struct.new(:recording_studio_user).new(pair_config)
    app_config = Struct.new(:x).new(xcfg)
    ...
    assert_equal 15, RecordingStudioUser.configuration.timeout
  end
  ...
  def test_load_config_swallow_each_pair_errors
    ...
    xcfg = Struct.new(:recording_studio_user).new(bad_pair_config)
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      def config_for(_name)
        { api_key: "ok" }
    ...
    assert_equal "ok", RecordingStudioUser.configuration.api_key
  end
  ...
  def test_load_config_is_noop_without_config_sources
    ...
    assert_nil RecordingStudioUser.configuration.api_key
    assert_equal 5, RecordingStudioUser.configuration.timeout
    assert_equal false, RecordingStudioUser.configuration.enable_feature_x
  end
  ...
  def test_load_config_ignores_non_enumerable_yaml_and_merge_errors
    ...
    xcfg = Struct.new(:recording_studio_user).new({ timeout: 22 })
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      attr_accessor :yaml
    ...
    assert_equal 22, RecordingStudioUser.configuration.timeout
  end
  ...
  def test_apply_extension_initializers_register_active_support_on_load_callbacks
    ...
    RecordingStudioUser::Engine.stub(:config, config_stub) do
      find_initializer("recording_studio_user.apply_model_extensions").block.call
      find_initializer("recording_studio_user.apply_controller_extensions").block.call
    end
  ...
  def test_model_extension_initializer_skips_abstract_models
    ...
    with_temporary_nested_constant(:ActiveRecord, :Base, active_record_base) do
      RecordingStudioUser::Engine.stub(:apply_model_extensions, ->(model) { applied << model }) do
  ...
  def test_controller_extension_initializer_applies_all_controllers
    ...
    with_temporary_nested_constant(:ActionController, :Base, action_controller_base) do
      RecordingStudioUser::Engine.stub(:apply_controller_extensions, ->(controller) { applied << controller }) do
        to_prepare_blocks.first.call
      end
    end
  ...
  def test_apply_model_extensions_adds_registered_methods_once
    ...
    RecordingStudioUser.configuration.hooks.extend_model(:ExampleRecord) do
      def template_extension_method
        :applied
      end
    end

    RecordingStudioUser::Engine.apply_model_extensions(model_class)
    RecordingStudioUser::Engine.apply_model_extensions(model_class)
  ...
  def test_apply_controller_extensions_matches_demodulized_name
    ...
    RecordingStudioUser.configuration.hooks.extend_controller(:DashboardController) do
      def template_controller_extension
        :applied
      end
    end

    RecordingStudioUser::Engine.apply_controller_extensions(controller_class)
  ...
  def test_apply_extensions_flattens_compacts_and_tracks_identity
    ...
    RecordingStudioUser::Engine.send(:apply_extensions, target, [nil, [extension, extension]])

    assert_equal :generated, target.new.generated_method
    assert_equal true, target.instance_variable_get(:@recording_studio_user_applied_extensions).compare_by_identity?
  end

  def test_apply_extensions_returns_without_target
    assert_nil RecordingStudioUser::Engine.send(:apply_extensions, nil, [])
  end
  ...
  def test_extension_keys_for_includes_demodulized_name
    ...
    assert_equal expected_keys, RecordingStudioUser::Engine.send(:extension_keys_for, namespaced)
  end
  ...
  def test_extension_keys_for_removes_duplicate_names
    ...
    assert_equal [:ReportsController], RecordingStudioUser::Engine.send(:extension_keys_for, plain)
  end
  ...
  def find_initializer(name)
    RecordingStudioUser::Engine.initializers.find { |initializer| initializer.name == name }
  end
end
