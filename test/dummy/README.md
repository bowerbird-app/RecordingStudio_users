...
- Login and sign up use the `0.6.3` viewport-centered Flatpack chrome (no Card, no Remember me, no seed Badge). Primary `/users/sign_in` and `/users/sign_up` are password forms only; email OTP lives on separate routes (`/users/sign_in/otp`, `/users/sign_up/otp`). Continue-with sits under a Flatpack Divider labeled Or. Dummy development credentials keep Google live. Recapture: `doc/review/users_sign_in.png`, `doc/review/users_sign_up.png`.
