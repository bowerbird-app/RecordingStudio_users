...
- Home (`/`) and the `/docs/*` debug pages use `layouts/flat_pack_sidebar`: left sidebar with Install, Config, Routes and integrations, diagnostics, Letters, My profile, and Sign out, plus a top nav with the Root Switchable dropdown. That chrome is for host debugging, not the Profile product UI.
