...
- Dummy installs released `recording_studio_terms_and_conditions` `v0.7.0` (Publishable `v0.3.1`, Flatpack `v0.1.196`) as an **integration demo only**. Production Users does not depend on TnC. The dummy Gemfile path gem satisfies TnC’s `recording_studio_user >= 0.12.1` requirement. Seeds publish one live Terms record under **My workspace** and write seed receipts for the named accounts. Users detects that gem and the live record, so create-password shows the xs muted “By continuing…” notice even when the current workspace has no live Terms, and Sign up writes an Acceptance with provenance `continue_notice`. Dummy uses the gem Accept screen (continue-notice + **Continue**, no Terms link on Accept). Accept titles the page **We have updated our terms and conditions.** The gate no longer flashes a duplicate notice.
