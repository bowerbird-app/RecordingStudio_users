...
- Login and sign up use the gem auth screens via `recording_studio_user_auth_for :users` (`0.8.0` chrome). Primary `/users/sign_in` and `/users/sign_up` are password forms; email OTP lives on separate routes (`/users/sign_in/otp`, `/users/sign_up/otp`). Continue-with sits under a Flatpack Divider labeled Or. Dummy development credentials keep Google live. Recapture: `doc/review/users_sign_in.png`, `doc/review/users_sign_up.png`.
