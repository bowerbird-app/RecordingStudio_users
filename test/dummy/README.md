...
- Login and sign up use the gem auth screens via `recording_studio_user_auth_for :users`. Primary `/users/sign_in` and `/users/sign_up` are email-first (**Continue with email**), then password or OTP on the second screen from `primary_login_type`. Continue-with sits under a Flatpack Divider labeled Or. When Site Settings has a square logo on the Admin root, that mark sits above the auth title. Dummy development credentials keep Google live.
