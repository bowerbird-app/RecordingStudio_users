...
- Dummy installs released `recording_studio_terms_and_conditions` `v0.6.2` (and Publishable `v0.3.1`) as an **integration demo only**. Production Users does not depend on TnC. The dummy Gemfile path gem satisfies TnC’s `recording_studio_user >= 0.12.1` requirement. Seeds publish one live Terms record under **My workspace** and write seed receipts for the named accounts. Users detects that gem and the live record, so create-password shows the xs muted “By continuing…” notice even when the current workspace has no live Terms, and Sign up writes an Acceptance with provenance `continue_notice`. Dummy overrides the Agree screen so re-accept no longer shows an on-page “Terms updated” alert.
...
- Profile show/edit are PageNav back/close only. The right slot stays empty. There is no Access control, sidebar Sign out, or Root Switchable on those screens. Show puts Edit in the page title and one elevated identity Card. A two-column Grid wraps the Card only (desktop width cap). Inside the card, Avatar sits above unlabeled name, email, and time zone in one column. Empty photos show Avatar's person icon. Edit hosts a `profile-photo` Turbo frame with Flatpack Avatar (`attachment_preview_url`, `size: :"2xl"`) plus Attachable `render_attachment_file_button` for Add/Change. Dummy pins Flatpack `v0.1.147`. An `mb-8` wrapper around the frame separates the photo row from First name. Edit puts the form in one cell of a two-column Flatpack Grid (the other cell stays empty) so fields do not stretch full-bleed on a wide screen. Each field is still its own full-width row. Update profile and Cancel are separate Flatpack buttons, not a ButtonGroup.
