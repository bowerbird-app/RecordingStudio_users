...
- Dummy installs released `recording_studio_terms_and_conditions` `v0.6.1` (and Publishable `v0.3.1`) as an **integration demo only**. Production Users does not depend on TnC. The dummy Gemfile path gem satisfies TnC’s `recording_studio_user >= 0.12.1` requirement. Seeds publish one live Terms record under **My workspace** and write seed receipts for the named accounts. Users detects that gem and the live record, so create-password shows the xs muted “By continuing…” notice and Sign up writes an Acceptance with provenance `continue_notice`.
