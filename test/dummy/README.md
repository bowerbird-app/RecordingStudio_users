...
- Dummy OmniAuth reads `Rails.application.credentials.dig(:omniauth, …)` first, then ENV, then `test-…` fallbacks. Encrypted development credentials live in `config/credentials/development.yml.enc`. For this dummy app, `config/credentials/development.key` is committed so the reference Google client can be decrypted.
...
3. Save `config/credentials/development.key` (do not commit it), then:
...
4. Start the dummy **without** `OMNIAUTH_TEST_MODE`. That env var (or `Rails.env.test?`) turns on OmniAuth mocks so Continue with Google never hits Google.

```bash
cd test/dummy
unset OMNIAUTH_TEST_MODE
