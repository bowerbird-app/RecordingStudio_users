...
- Dummy OmniAuth reads `Rails.application.credentials.dig(:omniauth, …)` first, then ENV, then `test-…` fallbacks. Encrypted development credentials live in `config/credentials/development.yml.enc`. The matching `development.key` is gitignored.
