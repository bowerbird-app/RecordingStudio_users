...
- Login and sign up use the gem auth screens via `recording_studio_user_auth_for :users`. Primary `/users/sign_in` and `/users/sign_up` are email-first (**Continue with email**), then password or OTP on the second screen from `primary_login_type`. Continue-with sits under a Flatpack Divider labeled Or. When Site Settings has a logo on the Admin root, that mark sits above the auth title. Dummy keeps `config.auth_logo` at the default `:square`. Set `:wide` to show the seeded wide logo instead. `doc/review/users_sign_in_wide.png` is that flip. Dummy development credentials keep Google live.
- Dummy vendors and mounts `recording_studio_site_settings` at `/recording_studio_site_settings`, registers its recordable, and opens Admin on the Site section (`root_section: :site_settings`). Seeds name the site **Studio** and attach square/wide logos from `db/seeds/`.
- Dummy installs released `recording_studio_terms_and_conditions` `v0.6.4` (Publishable `v0.3.1`, Flatpack `v0.1.195`) as an **integration demo only**. Production Users does not depend on TnC. The dummy Gemfile path gem satisfies TnC’s `recording_studio_user >= 0.12.1` requirement. Seeds publish one live Terms record under **My workspace** and write seed receipts for the named accounts. Users detects that gem and the live record, so create-password shows the xs muted “By continuing…” notice even when the current workspace has no live Terms, and Sign up writes an Acceptance with provenance `continue_notice`. Dummy uses the gem Accept screen (continue-notice + **Continue**). Re-gate flashes “We've updated our Terms and Conditions” and titles the page **We have updated our terms and conditions.**
- Product surfaces (Profile, Attachable leftovers, Root Switchable) use Recording Studio's default layout (`RecordingStudio::UsesDefaultLayout`). Sign in, sign up, password reset, and resend confirmation use the gem layout `recording_studio_user/auth`, which owns the full-viewport centering with `html data-theme="rounded"`. Dummy does not own `recording_studio/default_layout`. A `recording_studio/_default_layout_head` hook sets the same theme on product pages so Flatpack primary tokens resolve on `:root`.
- Home (`/`) and the `/docs/*` debug pages use `layouts/flat_pack_sidebar`: left sidebar with Install, Config, Routes and integrations, diagnostics, Letters, Notifications, Notification settings, Push devices, My profile, and Sign out, plus a top nav with the notification menu and Root Switchable dropdown. That chrome is for host debugging, not the Profile product UI.
- Dummy mounts `recording_studio_notifications` at `/notifications`, the email channel (no screens), and `recording_studio_notifications_push` at `/notifications/push`. OTP types are registered by Users; email still lands in Letters.
...
- Profile show/edit are PageNav back/close only. The right slot stays empty. There is no Access control, sidebar Sign out, or Root Switchable on those screens. Show puts Edit in the page title and one elevated identity Card. A two-column Grid wraps the Card only (desktop width cap). Inside the card, Avatar sits above unlabeled name, email, and time zone in one column. Empty photos show Avatar's person icon. Edit hosts a `profile-photo` Turbo frame with Flatpack Avatar (`attachment_preview_url`, `size: :"2xl"`) plus Attachable `render_attachment_file_button` for Add/Change. Dummy pins Flatpack `v0.1.195`. An `mb-8` wrapper around the frame separates the photo row from First name. Edit puts the form in one cell of a two-column Flatpack Grid (the other cell stays empty) so fields do not stretch full-bleed on a wide screen. Each field is still its own full-width row. Update profile and Cancel are separate Flatpack buttons, not a ButtonGroup.
