...
- Profile **show** renders one Flatpack elevated Card: Avatar (image or person-icon fallback) with unlabeled name, email, and time zone. An inner Grid `cols: 2` puts Avatar left of the stack on desktop and stacks at phone width. **Edit** calls Attachable `render_parent_attachment(recording, return_to:, shape: :circle, size: :xl)` for Avatar plus the icon-only camera. These screens are not a gallery or library.
...
- Profile show puts **Edit** in the PageTitle actions slot (not in the card) and drops the subtitle. The identity Card sits in a Flatpack Grid `cols: 2` so it occupies one cell on a wide screen. No city field. Edit wraps the Attachable photo slot, stacked fields, and Update / Cancel in a Flatpack Grid `cols: 2` so the form occupies one cell (width constraint). Fields stay full-width rows — not two columns of first/last name. Update profile and Cancel are two separate Flatpack buttons, not a ButtonGroup. Edit keeps the plain subtitle: "Change your name, time zone, or photo."
...
- Use `attach_profile_image!` or `recording.import_attachment` to place one image under the Profile recording. On Edit Profile call `render_parent_attachment(recording, return_to: edit_profile_path, shape: :circle, size: :xl)`. On My Profile render one elevated Card (Avatar plus unlabeled name, email, and time zone) in a Grid `cols: 2` width cap. Do not compose a Users-owned photo form or link replace to `attachments#show`.
