...
- Dummy app installs released `recording_studio_terms_and_conditions` `v0.6.4`
...
### Upgrade notes
- Bump to `0.12.2`.
- No host change. Install TnC `0.6.4+` and publish live Terms. Create-password
...
- Dummy-only pin: TnC `v0.6.4`, Flatpack `v0.1.195`, and Publishable `v0.3.1`
  in `test/dummy`.
...
- When `auth_logo` is `:wide`, `auth_site_logo` renders a proportioned `<img>`
  ...
  square Avatar. The wide `<img>` uses `max-h-16` (64px, same height as square
  Avatar `:xl` `h-16`). A blank wide slot omits the mark. There is no broken
  image and no square fallback. Dummy keeps the square default. Wide review
  shots are `doc/review/users_sign_in_wide.png` and
  `doc/review/users_sign_up_wide.png`.
...
### Added
- `.cursor/install.sh` provisions a cold image and skips apt, ruby-build,
  db:prepare, and tailwind when Ruby, bundle, and Postgres are already usable.
  A skippable provision failure does not fail the Build. Fetch-skills always
  runs last, not with `|| true`.
- `.cursor/start.sh` starts PostgreSQL on each environment boot.
- Dummy Cloud Agent terminals for the rails server and Tailwind watch.
