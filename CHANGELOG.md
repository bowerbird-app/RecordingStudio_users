...
- Dummy and development Gemfiles pin `recording_studio_attachable` to branch `cursor/file-only-replace-path-a5db` at `62c2f944d1ed206c84c80275851db0aee8e3306a` (0.5.0) so `render_attachment_file_button` owns Add/Change text buttons. The gemspec requires `~> 0.5`. Do not merge this Users PR until Attachable 0.5.0 is tagged.
