...
- Dummy Devise login and sign-up use the Flatpack auth stack without a Card: `Grid` `cols: 1`, `justify: :center`, `max: :sm` → title → fields → primary button → cross-link → `Divider` (`label: "Or"`) → full-width secondary Continue-with buttons (provider logo via `icon:`). Login omits Remember me. Seed hint sits under the login form, not inside it. Dummy pins Flatpack branch `cursor/grid-justify-max-8d62` (gemspec `~> 0.1.144`) until that Grid API is tagged. Button `icon:` SVG logos use a small engine shim until Flatpack Button mirrors List::Item.

### Upgrade notes
- Bump to `0.6.0`. Requires Flatpack `~> 0.1.144`. Until Flatpack Grid `justify` / `max` is on a tagged release, pin the Flatpack git branch `cursor/grid-justify-max-8d62` (see Flatpack PR #169). Do not invent a Flatpack tag.
