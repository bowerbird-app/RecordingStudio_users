...
- Dummy Devise login and sign-up use the Flatpack auth stack without a Card: Grid `cols: 2` width cap → title → fields → primary button → cross-link → `Divider` (`label: "Or"`) → full-width secondary Continue-with buttons (provider logo via `icon:`). Login omits Remember me. Seed hint sits under the login form, not inside it. Requires Flatpack `~> 0.1.142` (Divider). Button `icon:` SVG logos use a small engine shim until Flatpack Button mirrors List::Item.
