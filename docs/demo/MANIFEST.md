...
Dummy installs released `recording_studio_terms_and_conditions` `v0.6.4` and Flatpack `v0.1.196`. Users detects that gem and pending live Terms on create-password. TnC `Gate.root_for_signup` falls back to the first live-Terms root when the current workspace has none. Production Users stays independent of a gemspec pin. Dummy uses the gem Accept screen (continue-notice + **Continue**).
