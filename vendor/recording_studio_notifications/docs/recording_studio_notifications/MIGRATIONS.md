...
This guide explains how to work with database migrations in RecordingStudioNotifications.

---

## Installing Migrations in a Host App

RecordingStudioNotifications includes a migrations generator that copies engine migrations to your host application with proper timestamps.
...
```bash
rails generate recording_studio_notifications:migrations --pretend
```
...
```bash
rails generate recording_studio_notifications:migrations --no-skip-existing --force
```
...
```bash
# From the gem root
touch db/migrate/$(date +%Y%m%d%H%M%S)_create_recording_studio_notifications_pages.rb
```
...
1. **Use UUID primary keys** for PostgreSQL compatibility:
   ...
   ```ruby
    class CreateRecordingStudioNotificationsPages < ActiveRecord::Migration[8.1]
     def change
          create_table :recording_studio_notifications_pages, id: :uuid do |t|
         ...
         t.timestamps
          ...
          add_index :recording_studio_notifications_pages, [:workspace_id, :slug], unique: true
     end
   end
   ```
...
2. **Prefix table names** with the gem name to avoid conflicts:
   ...
   ```ruby
   # Good: recording_studio_notifications_pages
   # Bad: pages
   ```
...
4. **Add indexes** for frequently queried columns:
   ...
   ```ruby
   add_index :recording_studio_notifications_pages, :workspace_id
   add_index :recording_studio_notifications_pages, :published
   ```
...
db/
└── migrate/
   └── 20250101000001_create_recording_studio_notifications_pages.rb
```
...
- Class names change (e.g., `CreateRecordingStudioNotificationsPages` → `CreateMyEnginePages`)
- Table names change (e.g., `recording_studio_notifications_pages` → `my_engine_pages`)
