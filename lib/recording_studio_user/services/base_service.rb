  ...
  module Services
    ...
    class BaseService
      ...
      def run_before_hooks
        ...
        RecordingStudioUser::Hooks.run(:before_service, self.class, service_args)
      end
      ...
      def run_after_hooks(result)
        ...
        RecordingStudioUser::Hooks.run(:after_service, self.class, result)
      end
      ...
      def run_with_around_hooks
        if hooks_enabled? && RecordingStudioUser.configuration.hooks.registered?(:around_service)
          RecordingStudioUser::Hooks.run_around(:around_service, self) { perform }
        else
          perform
        end
      end

      # Check if hooks are enabled (RecordingStudioUser is loaded and configured)
      #
      # @return [Boolean]
      def hooks_enabled?
        defined?(RecordingStudioUser) &&
          RecordingStudioUser.respond_to?(:configuration) &&
          RecordingStudioUser.configuration.respond_to?(:hooks)
      end
