    ...
    class Mapper
      ...
      def recording_studio_user_auth_for(name = :users, path: nil, **)
        ...
        devise_scope singular.to_sym do
          delete "#{mount_path}/sign_out", to: "devise/sessions#destroy",
                                          as: :"destroy_#{singular}_session"
          get "#{mount_path}/password/new", to: "recording_studio_user/auth/passwords#new",
                                            as: :"new_#{singular}_password"
          post "#{mount_path}/password", to: "recording_studio_user/auth/passwords#create",
                                         as: :"#{singular}_password"
        end
      end
    end
  end
